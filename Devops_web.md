# System Administrator, DevOps Practitioners booklet

Developers and Open Source authors now have many services offering free tiers, but finding them all takes time to make informed decisions.
现在开发者和开源软件作者有很多服务提供免费套餐，但要找到所有这些服务并做出明智的选择需要时间。

This is a list of software (SaaS, PaaS, IaaS, etc.) and other offerings with free developer tiers.
这是包含免费开发者级别的软件（SaaS、PaaS、IaaS 等）和其他产品的列表。
...
**注意**：此列表仅适用于“即服务”（as-a-Service）产品，不适用于自托管软件。要符合入选条件，服务必须提供免费套餐，而不仅仅是免费试用。如果免费套餐按时间段计费，则必须至少持续一年。我们还会从安全性的角度评估免费套餐，因此支持单点登录（SSO）的服务可以接受，但不会接受仅在付费套餐中提供 TLS 加密的服务。

# Table of Contents  ＃ 目录

- [System Administrator, DevOps Practitioners booklet](#system-administrator-devops-practitioners-booklet)
- [Table of Contents  ＃ 目录](#table-of-contents---目录)
  - [Major Cloud Providers  ## 主要云服务提供商](#major-cloud-providers---主要云服务提供商)
  - [Cloud management solutions](#cloud-management-solutions)
  - [云管理解决方案](#云管理解决方案)
  - [Source Code Repos  ## 源代码库](#source-code-repos---源代码库)
  - [APIs, Data, and ML](#apis-data-and-ml)
  - [API、数据和机器学习](#api数据和机器学习)
  - [Artifact Repos  ## 制品仓库](#artifact-repos---制品仓库)
  ...
  - [代码搜索和浏览](#代码搜索和浏览)
  - [CI and CD  ## CI 和 CD](#ci-and-cd---ci-和-cd)
  - [Testing  ## 测试](#testing---测试)
  - [Security and PKI  ## 安全性和公钥基础设施](#security-and-pki---安全性和公钥基础设施)
  ...
  - [移动应用分发和反馈](#移动应用分发和反馈)
  - [Management System  ## 管理系统](#management-system---管理系统)
  - [Messaging and Streaming](#messaging-and-streaming)
  - [消息传递和流媒体](#消息传递和流媒体)
  - [Log Management  ## 日志管理](#log-management---日志管理)
...
## Major Cloud Providers  ## 主要云服务提供商
...
* [亚马逊云服务](https://aws.amazon.com) * [CloudFront](https://aws.amazon.com/cloudfront/) - 每月 1TB 出站流量和 200 万次函数调用 * [CloudWatch](https://aws.amazon.com/cloudwatch/) - 10 个自定义指标和 10 个警报 * [CodeBuild](https://aws.amazon.com/codebuild/) - 每月 100 分钟构建时间 * [CodeCommit](https://aws.amazon.com/codecommit/) - 5 个活跃用户，50GB 存储空间和每月 10000 次请求 * [CodePipeline](https://aws.amazon.com/codepipeline/) - 每月 1 个活跃管道 * [DynamoDB](https://aws.amazon.com/dynamodb/) - 25GB NoSQL 数据库 * [EC2](https://aws.amazon.com/ec2/) -每月 750 小时 t2.micro 或 t3.micro（12 个月）。每月 100GB 出站流量 * [EBS](https://aws.amazon.com/ebs/) - 每月 30GB 通用型 (SSD) 或磁性存储（12 个月） * [弹性负载均衡](https://aws.amazon.com/elasticloadbalancing/) - 每月 750 小时（12 个月） * [RDS](https://aws.amazon.com/rds/) - 每月 750 小时 db.t2.micro、db.t3.micro 或 db.t4g.micro 存储，20GB 通用型 (SSD) 存储，20GB 存储备份（12 个月） * [S3](https://aws.amazon.com/s3/) - 5GB 标准对象存储，2 万次 GET 请求和 2 千次 PUT 请求（12 个月） * [Glacier](https://aws.amazon.com/glacier/) - 10GB 长期对象存储 * [Lambda](https://aws.amazon.com/lambda/) - 每月 100 万次请求 * [SNS](https://aws.amazon.com/sns/) - 每月 100 万次发布 * [SES](https://aws.amazon.com/ses/) - 每月 3000 条消息（12 个月） * [SQS](https://aws.amazon.com/sqs/) - 100 万次消息队列请求 * 完整详细列表 - https://aws.amazon.com/free/
...
* [Oracle 云](https://www.oracle.com/cloud/) * 计算 - 2 个基于 AMD 的计算虚拟机，每个虚拟机配备 1/8 个 OCPU 和 1 GB 内存 - 2 个基于 Arm 的 Ampere A1 内核和 12 GB 内存，可作为 1 个虚拟机或最多 2 个虚拟机使用 - 当实例被视为空闲时，将被回收。* 块存储卷 - 2 个卷，总计 200 GB（用于计算）* 对象存储 - 10 GB * 负载均衡器 - 1 个实例，带宽 10 Mbps * 数据库 - 2 个数据库，每个数据库 20 GB * 监控 - 5 亿个数据摄取点，10 亿个数据检索点 * 带宽 - 每月 10 TB 出站流量，速度限制为基于 x64 的虚拟机上 50 Mbps 的速度，基于 ARM 的虚拟机上 500 Mbps 的速度 * 核心数 * 公网 IP - 2 个 IPv4 地址供虚拟机使用，1 个 IPv4 地址供负载均衡器使用 * 通知 - 每月 100 万条通知，每月发送 1000 封电子邮件 * 完整详细列表 - https://www.oracle.com/cloud/free/
...
* [IBM Cloud](https://www.ibm.com/cloud/free/) * Cloudant 数据库 - 1 GB 数据存储 * Db2 数据库 - 100MB 数据存储 * API Connect - 每月 50,000 次 API 调用 * 可用性监控 - 每月 300 万个数据点 * 日志分析 - 每日 500MB 日志 * 完整详细列表 - https://www.ibm.com/cloud/free/
...
* Brainboard - 协作式解决方案，可直观地构建和管理端到端的云基础设施。* Cloud 66 - 为个人项目提供免费版本（包含一个部署服务器和一个静态站点），Cloud 66 为您提供在任何云平台上构建、部署和扩展应用程序所需的一切，无需操心服务器相关事宜。* Deployment.io - Deployment.io 帮助开发人员自动化在 AWS 上的部署。在我们的免费套餐中，开发人员（单用户）可以部署无限量的静态站点、Web 服务和环境。我们每月提供 10 次免费作业执行，免费套餐包含预览和自动部署功能。* Pulumi - 现代化的基础设施即代码平台，允许您使用熟悉的编程语言和工具来构建、部署和管理云基础设施。 * [scalr.com](https://scalr.com/) - Scalr 是一款 Terraform 自动化与协作 (TACO) 产品，用于更好地协作和自动化 Terraform 管理的基础设施和配置。它完全支持 Terraform CLI、OPA 集成和分层配置模型。无需支付 SSO 费用。所有功能均已包含。每月最多可免费运行 50 次。
...
* [Gemfury](https://gemfury.com) - 为 Maven、PyPI、NPM、Go Module、NuGet、APT 和 RPM 仓库提供私有和公共构件仓库。公共项目免费。* [jitpack.io](https://jitpack.io/) - GitHub 上的 JVM 和 Android 项目的 Maven 仓库，公共项目免费。* [paperspace](https://www.paperspace.com/) - 构建和扩展 AI 模型，开发、训练和部署 AI 应用，免费方案：公共项目、5GB 存储空间、基础实例。* [RepoFlow](https://repoflow.io) - RepoFlow 简化包管理，支持 npm、PyPI、Docker、Go、Helm 等。免费试用，云端提供 10GB 存储空间、10GB 带宽、100 个包和无限用户，也可自托管，仅限个人使用。 * [RepoForge](https://repoforge.io) - 私有云托管的 Python、Debian、NPM 包和 Docker 镜像仓库。为开源/公共项目提供免费方案。* [repsy.io](https://repsy.io) - 1 GB 免费私有/公共 Maven 仓库。
...
* [Appinvento](https://appinvento.io/) - 一款免费的无代码应用构建器。它提供对自动生成的后端源代码的完全访问权限，并允许创建无限的 API 和路由。免费套餐包含三个项目和五个表格。* [DhiWise](https://www.dhiwise.com/) - 将 Figma 设计转换为动态的 Flutter 和 React 应用。其代码生成技术旨在优化工作流程，从而构建可用于生产环境的移动和 Web 应用。* [Karbon Sites](https://www.karbonsites.space) - 一款 AI 驱动的网站构建器和编辑器，可根据文本提示、草图或简历生成可用于生产环境的前端代码。功能包括原生 Android (APK) 导出，以及每月 5 次生成的免费套餐（可通过自定义 Gemini API 密钥无限生成）。* [Metalama](https://www.postsharp.net/metalama) - 一款专门针对 AC# 的工具，可在编译期间动态生成样板代码，以保持源代码的整洁。它对开源项目免费；其面向商业用途的免费套餐包含最多三个功能。* [Supermaven](https://www.supermaven.com/) - 一款适用于 VS Code、JetBrains 和 Neovim 的高速 AI 代码补全插件。免费套餐提供无限次的内联补全，并专注于超低延迟。* [v0.dev](https://v0.dev/) - 由 Vercel 开发，v0 使用 shadcn/ui 和 Tailwind CSS 生成可直接复制粘贴的 React 代码。它采用积分制，提供 1200 个初始积分和每月 200 个免费积分。
...
* [CodeKeep](https://codekeep.io) - 代码片段版的 Google Keep。整理、发现和分享代码片段，配备强大的代码截图工具，提供预设模板和链接功能。* [libraries.io](https://libraries.io/) - 为 32 种不同的包管理器提供搜索和依赖更新通知，开源用户免费使用。* [Namae](https://namae.dev/) - 搜索 GitHub、GitLab、Heroku、Netlify 等众多网站，查看您的项目名称是否可用。* [tickgit.com](https://www.tickgit.com/) - 显示 `TODO` 注释（和其他标记），以便识别值得回头改进的代码区域。
...
* [Appho.st](https://appho.st) - 移动应用托管平台。免费套餐包含五个应用、每月 50 次下载，以及最大 100 MB 的文件大小。* [Diawi](https://www.diawi.com) - 直接将 iOS 和 Android 应用部署到设备。免费套餐：应用上传、密码保护链接、1 天有效期、10 次安装。* [GetUpdraft](https://www.getupdraft.com) - 分发用于测试的移动应用。免费套餐包含一个应用项目、三个应用版本、500 MB 存储空间和每月 100 次应用安装。* [InstallOnAir](https://www.installonair.com) - 通过 OTA 分发 iOS 和 Android 应用。免费套餐：无限上传、私密链接、访客 2 天有效期，注册用户 60 天有效期。 * [Loadly](https://loadly.io) - iOS 和 Android 测试版应用分发服务提供完全免费的服务，包括无限下载、高速下载和无限上传。
...
* [bitnami.com](https://bitnami.com/) - 在 IaaS 上部署已准备好的应用程序。免费管理 1 个 AWS 微型实例。* [Esper](https://esper.io) - 适用于 Android 设备的 MDM 和 MAM，支持 DevOps。一个用户许可证，免费管理 100 台设备，并提供 25 MB 的应用程序存储空间。* [jamf.com](https://www.jamf.com/) - 适用于 iPad、iPhone 和 Mac 的设备管理，免费管理 3 台设备。* [Miradore](https://miradore.com) - 设备管理服务。随时掌握您的设备群的最新动态，并免费保护无限数量的设备。免费套餐提供基本功能。* [moss.sh](https://moss.sh) - 帮助开发者部署和管理他们的 Web 应用程序和服务器。每月最多可免费进行 25 次 Git 部署。* [ploi.io](https://ploi.io/) - 服务器管理工​​具，可轻松管理和部署您的服务器和网站。免费管理一台服务器。 * [runcloud.io](https://runcloud.io/) - 主要面向 PHP 项目的服务器管理服务。免费支持最多 1 台服务器。* [serveravatar.com](https://serveravatar.com) - 管理和监控基于 PHP 的 Web 服务器，并提供自动化配置。免费支持 1 台服务器。* [xcloud.host](https://xcloud.host) - 拥有用户友好界面的服务器管理和部署平台。提供免费套餐，支持 1 台服务器。
...
* [bugfender.com](https://bugfender.com/) - 每天最多可免费记录 10 万行日志，保留 24 小时。* [log.dog](https://log.dog/) - LogDog 是一款远程调试/日志记录 SDK（支持 iOS 和 Android），带有 Web 用户界面。它可以实时捕获所有日志、请求和事件，并允许拦截它们。每月最多可免费记录 100MB 的日志。* [logflare.app](https://logflare.app/) - 每个应用每月最多可免费记录 12,960,000 条日志，保留 3 天。* [logtail.com](https://logtail.com/) - 基于 ClickHouse 的 SQL 兼容日志管理工具。每月最多可免费记录 1GB 的日志，保留 3 天。* [logzab.com](https://logzab.com/) - 审计跟踪管理系统。每月最多可免费记录 1,000 条用户活动日志，保留 1 个月，最多可用于 5 个项目。 * [ManageEngine Log360 Cloud](https://www.manageengine.com/cloud-siem/) - 由 ManageEngine 提供支持的日志管理服务。免费套餐提供 50 GB 存储空间，15​​ 天存储保留期和 7 天搜索期。* [openobserve.ai](https://openobserve.ai/) - 每月 200 GB 免费摄取量，15 天保留期。* [Smart Grow Logs](https://logs.smart-grow.app/) - 集中式日志管理平台，提供端到端加密、实时警报和多平台 SDK。免费套餐包含每天最多 3,000 条日志。
...
* [algolia.com](https://www.algolia.com/) - 提供托管搜索解决方案，具备拼写容错、相关性和 UI 库，可轻松创建搜索体验。免费的“Build”套餐包含 100 万份文档和每月 1 万次搜索。此外，还免费提供 [开发者文档搜索](https://docsearch.algolia.com/)。* [bonsai.io](https://bonsai.io/) - 免费提供 1 GB 内存和 1 GB 存储空间。* [CommandBar](https://www.commandbar.com/) - 统一搜索栏即服务，基于 Web 的 UI 小部件/插件，允许用户在您的产品内搜索内容、导航、功能等，从而提高产品发现率。每月活跃用户数上限为 1000 人，命令数量不限。* [searchly.com](http://www.searchly.com/) - 免费提供 2 个索引和 20 MB 存储空间。
